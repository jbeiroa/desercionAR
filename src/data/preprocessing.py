...
def join_individuals_households(
    ...
    if on is None:
        on = join_features
    merged_df = pd.merge(individual_df, household_df, on=on, how=how, suffixes=suffixes)
    return merged_df


def create_binary_feature(
    data: pd.DataFrame, feature_name: str, condition: pd.Series
) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    if idf is not None and hdf is not None:
        ...
        merged_df = create_binary_feature(
            merged_df, "is_employed", merged_df["ESTADO"] == 1
        )
