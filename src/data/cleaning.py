...
def select_features(
    data: pd.DataFrame | None, column_list: list, id_columns: list | None = None
) -> pd.DataFrame:
...
def filter_by_columns(
    data: pd.DataFrame, query_filter: str, column_list: list | pd.Index | None = None
) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    filtered_df = filter_by_columns(
        df, "CH04 == 1 and ESTADO == 1", id_individual_features + id_features
    )
